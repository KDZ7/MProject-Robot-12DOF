...
def generate_launch_description():
    cerbere_pkg_path = get_package_share_directory("cerbere")
    ros_gz_sim_pkg_path = get_package_share_directory("ros_gz_sim")
    ros_gz_bridge_pkg_path = get_package_share_directory("ros_gz_bridge")
    config_path = os.path.join(cerbere_pkg_path, "config", "config.yaml")
    config_ros2_control_path = os.path.join(cerbere_pkg_path, "config", "ros2_control.yaml")

    with open(config_path, "r") as yaml_file:
        config_data = yaml.safe_load(yaml_file)

    os.environ["GZ_SIM_RESOURCE_PATH"] = f"{cerbere_pkg_path}/models/worlds:{cerbere_pkg_path}/.."
    ...
    robot_state_publisher_node = Node(
        ...
        parameters=[
            {
                "robot_description": ParameterValue(Command(["xacro ", PathJoinSubstitution([cerbere_pkg_path, "urdf", config_data["robot_state_publisher"]["urdf"]])])),
    ...
    rviz_node = Node(
        ...
        name="rviz",
        output="screen",
        arguments=["-d", PathJoinSubstitution([cerbere_pkg_path, "config", config_data["rviz2"]["config"]])]
    )
    ...
    ros_gz_bridge_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([PathJoinSubstitution([ros_gz_bridge_pkg_path, "launch", "ros_gz_bridge.launch.py"])]),
        launch_arguments={
            "bridge_name": config_data["ros_gz_bridge"]["bridge_name"],
            "config_file": PathJoinSubstitution([cerbere_pkg_path, "config", config_data["ros_gz_bridge"]["config"]]),
        }.items()
    )
